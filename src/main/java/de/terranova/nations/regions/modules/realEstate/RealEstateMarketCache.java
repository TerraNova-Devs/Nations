...
public class RealEstateMarketCache {
  ...
  public static void upsertListing(UUID parent, HasRealEstateAgent estate) {
    List<HasRealEstateAgent> list =
        realEstateListings.computeIfAbsent(parent, k -> new ArrayList<>());
    list.removeIf(
        existing -> existing.getAgent().region.getId().equals(estate.getAgent().region.getId()));
    list.add(estate);
  }
  ...
  public static boolean hasListing(UUID parent, UUID agentId) {
    ...
    return list.stream().anyMatch(existing -> existing.getAgent().region.getId().equals(agentId));
  }
}
