...
public class OutpostRegion extends Region {
  ...
  public static OutpostRegion conditionCheck(Player p, String[] args) {
    ...
    if (abstand < 750) {
      p.sendMessage(
          Chat.errorFade(
              "Du bist zu nah an einer anderen Stadt, mindestens <#8769FF>750<#FFD7FE> Bl\u00F6cke Abstand muss eingehalten werden."));
      p.sendMessage(
          Chat.errorFade(
              String.format(
    ...
    if (abstand2 < 300) {
      p.sendMessage(
          Chat.errorFade(
              "Du bist zu nah an einem anderen Outpost, mindestens <#8769FF>300<#FFD7FE> Bl\u00F6cke Abstand muss eingehalten werden."));
      p.sendMessage(
          Chat.errorFade(
              String.format(
                  "Die n\u00E4chste Stadt ist <#8769FF>%s<#FFD7FE> meter von dir entfernt.",
