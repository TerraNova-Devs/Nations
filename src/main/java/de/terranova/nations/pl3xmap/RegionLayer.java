...
public class RegionLayer extends WorldLayer implements RegionListener {
  ...
  public static void updateRegion(SettleRegion region) {
    ...
    Polygon polygonMarker =
        new Polygon(
            "polygon" + region.getId(), new Polyline("line" + region.getId(), markerPoints));

    // 2) Build the tooltip (already done in your code)
    Collection<String> vices =
        region.getAccess().getEveryMemberNameWithCertainAccessLevel(AccessLevel.VICE);
    Collection<String> councils =
        region.getAccess().getEveryMemberNameWithCertainAccessLevel(AccessLevel.COUNCIL);
    String owner = "error";
    if (!region.getAccess().getEveryUUIDWithCertainAccessLevel(AccessLevel.MAJOR).isEmpty()) {
      UUID ownerId =
          region
              .getAccess()
              .getEveryUUIDWithCertainAccessLevel(AccessLevel.MAJOR)
              .iterator()
              .next();
    ...
    var nation =
        de.terranova.nations.NationsPlugin.nationManager.getNationBySettlement(region.getId());
    ...
    if (nation != null) {
      ...
      if (bannerItem != null && bannerItem.getType().name().contains("BANNER")) {
        ...
        if (dataUri != null) {
          ...
          bannerHtml =
              "<img src=\"" + dataUri + "\" style=\"opacity:1.0;\" width=\"44\" height=\"80\" />";
        }
      }
    ...
    Options optionsicon =
        Options.builder().tooltipContent(tooltip).tooltipDirection(Tooltip.Direction.TOP).build();
    ...
    markers.put(
        ...
        Marker.icon(
                "icon" + region.getId(),
                region.getLocation().x,
                region.getLocation().z,
                "castle",
                32)
  ...
  private static String buildProfessionsHtml(UUID settlementId) {
    ...
    for (String type : allTypes) {
      ...
      sb.append(
              "<span class=\"profession-name\" style=\"color:#DDD; font-family: minecraft; margin-right:8px;\">")
          .append(prettyName)
          .append("</span>");
  ...
  /**
   * Returns a hex color (e.g. "#E91E63") to use for completed circles of the given profession type.
   * Adjust as desired!
   */
