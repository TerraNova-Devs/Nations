...
public class TownPlayersGUI extends RoseGUI {
  ...
  private void registerPlayerSlots() {
    ...
    LinkedHashMap<UUID, AccessLevel> sortedAccessLevels =
        accessLevels.entrySet().stream()
            .sorted(
                (entry1, entry2) ->
                    Integer.compare(entry2.getValue().getWeight(), entry1.getValue().getWeight()))
            .collect(
                LinkedHashMap::new,
    ...
    sortedAccessLevels.forEach(
        (uuid, level) -> {
          ...
          meta.lore(
              List.of(Component.text("§7" + level.name()), Component.text("§8Loading skin...")));
  ...
  private void handlePlayerClick(
      InventoryClickEvent e, Player player, AccessControlled access, AccessLevel level, UUID uuid) {
    if (!Access.hasAccess(
        access.getAccess().getAccessLevel(player.getUniqueId()), AccessLevel.VICE)) {
      player.sendMessage(
          Chat.errorFade(
              "Du musst mindestens Vize sein um den Rang dieses Spielers ändern zu können."));
      return;
    }
    ...
    if (e.isRightClick()) {
      ...
      if (Access.hasAccess(
          access.getAccess().getAccessLevel(player.getUniqueId()), AccessLevel.MAJOR)) {
