...
public class NationGUI extends RoseGUI {
  ...
  public void onOpen(InventoryOpenEvent event) {
    ...
    if (inviteSettlementItem != null) {
      ...
      inviteSettlementItem.onClick(
          e -> {
            player.closeInventory();
            player.sendMessage(
                Chat.cottonCandy(
                    "Schreib '/nation invite <Stadt-Name>' um eine Stadt zur Nation einzuladen."));
          });
    }
    ...
    // Banner change
    boolean canEditBanner =
        (nation.getPlayerRank(player.getUniqueId()).getWeight()
    ...
    if (canEditBanner) {
      ...
      RoseItem changeBanner =
          new RoseItem.Builder()
              .copyStack(customBanner)
              .displayName(Chat.yellowFade("<b>Banner ändern"))
              .addLore(
                  Chat.cottonCandy("<i>Klicke, während du das gewünschte Banner in der Hand hast."))
              .build();

      addItem(28, changeBanner);

      changeBanner.onClick(
          e -> {
            ...
            if (handItem == null || !handItem.getType().toString().endsWith("BANNER")) {
              player.sendMessage(
                  Chat.errorFade("Du musst ein Banner in der Hand halten, um es zu setzen!"));
              return;
            }
