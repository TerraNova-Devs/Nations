...
public class NationMembersGUI extends RoseGUI {
  ...
  private void registerPlayerSlots() {
    ...
    for (Map.Entry<UUID, SettlementRank> settlement : nation.getSettlements().entrySet()) {
      ...
      if (settle.isPresent()) {
        ...
        if (access != null) {
          accessLevels.putAll(
              access.getAccessLevels().entrySet().stream()
                              ...
                              updatedMeta.lore(
                                  List.of(
                                      Component.text(
                                          "§7Stadt: "
                                              + StringUtils.capitalise(
                                                  RegionManager.retrievePlayersSettlement(
                                      ...
                                      Component.text(
                                          "§7Nationsrang: "
                                              + nation
                                                  .getPlayerRank(offlinePlayer.getUniqueId())
                                                  .name())));
  ...
  private void handlePlayerClick(InventoryClickEvent e, Player player, UUID uuid) {
    ...
    if (ownRank.getWeight() < NationPlayerRank.VICE_LEADER.getWeight()) {
      player.sendMessage(
          Chat.errorFade(
              "Du musst mindestens Vize-Anführer sein um den Rang dieses Spielers ändern zu können."));
      return;
    }
