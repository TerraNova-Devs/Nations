...
public class ItemTransfer {
  ...
  public static Integer charge(
      Player player, String itemString, int amount, boolean onlyFullCharge) {
  ...
  public static Integer credit(
      Player player, String itemString, int amount, boolean onlyFullCredit) {
    ItemStack item = resolveItem(itemString);
    ItemStack[] inventory = player.getInventory().getContents();
  ...
  private static ItemStack resolveItem(String itemString) {
    if (NexoItems.exists(itemString)) {
      ...
      throw new IllegalArgumentException(
          "Folgendes Item konnte nicht gefunden werden:" + itemString);
    }
  }
