...
public class AccessDAO {
  ...
  static {
    ...
    queries.put(
        "add",
        "INSERT INTO `access` (RUUID, PUUID, access) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE access = VALUES(access);");
  ...
  public static Map<UUID, AccessLevel> getMembersAccess(UUID ruuid) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
      ...
      while (rs.next()) {
        access.put(
            UUID.fromString(rs.getString("PUUID")), AccessLevel.valueOf(rs.getString("access")));
      }
  ...
  public static void changeMemberAccess(UUID ruuid, UUID puuid, AccessLevel access) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to change member access: " + puuid.toString());
    }
  }
