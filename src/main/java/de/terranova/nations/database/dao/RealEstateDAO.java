...
public class RealEstateDAO {
  ...
  static {
    queries.put("selectRealEstateById", "SELECT * FROM `nations_realestate` WHERE `RUUID` = ?;");
    queries.put(
        "upsertRealEstate",
    ...
    queries.put("removeRealEstate", "DELETE FROM `nations_realestate` WHERE `RUUID` = ?;");

    queries.put(
        "upsertHolding",
        "INSERT INTO `nations_holdings` (`PUUID`, `amount`) VALUES (?, ?) "
            + "ON DUPLICATE KEY UPDATE `amount` = VALUES(`amount`);");
    queries.put("removeHolding", "DELETE FROM `nations_holdings` WHERE `PUUID` = ?;");
    queries.put("selectHoldingByPUUID", "SELECT `PUUID`, `amount` FROM `nations_holdings`;");
  }
  ...
  public static void upsertRealEstate(String RUUID, RealEstateListing data) {
    ...
    try (Connection conn = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to upsert real estate entry: " + e.getMessage());
    }
  }
  ...
  public static void removeRealEstate(RealEstateAgent agent) {
    ...
    try (Connection conn = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to remove real estate entry: " + e.getMessage());
    }
  }
  ...
  public static RealEstateListing getRealEstateById(UUID ruuid) {
    ...
    try (Connection conn = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to fetch real estate by id: " + e.getMessage());
    }
    ...
    try (Connection conn = NationsPlugin.hikari.dataSource.getConnection();
      ...
      while (rs.next()) {
        try {
          ...
          NationsPlugin.plugin
              .getLogger()
              .warning("Skipping invalid PUUID in holdings: " + rs.getString("PUUID"));
        }
      }
    } catch (SQLException e) {
