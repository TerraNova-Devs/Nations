...
public class BankDAO {
  ...
  static {
    queries.put(
        ...
        "SELECT `user`, `credit` AS amount, `timestamp` AS date, `total` FROM `bank` WHERE `RUUID` = ? ORDER BY `timestamp` ASC LIMIT 50;");
    queries.put(
        "getBankCredit",
        "SELECT `user`, `credit` AS amount, `timestamp` AS date, `total` FROM `bank` WHERE `RUUID` = ? ORDER BY `timestamp` DESC LIMIT 1;");
    queries.put(
        "insert",
        "INSERT INTO `bank` (`RUUID`, `user`, `credit`, `timestamp`, `total`) VALUES (?, ?, ?, ?, ?);");
    queries.put("delete", "DELETE FROM `bank` WHERE `RUUID` = ?;");
    queries.put(
        "checkEntries",
        "DELETE FROM `bank` WHERE `RUUID` = ? AND `timestamp` NOT IN (SELECT `timestamp` FROM (SELECT `timestamp` FROM `bank` WHERE `RUUID` = ? ORDER BY `timestamp` DESC LIMIT 50) AS recent_timestamps) LIMIT 1;");
  }
  ...
  public List<Transaction> getLatestTransactions(UUID ruuid) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to fetch transactions for " + ruuid.toString());
    }
    return transactions;
  }
  ...
  public Optional<Transaction> getBankCredit(UUID ruuid) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to fetch bank credit for " + ruuid.toString());
    }
  ...
  public void insertTransaction(UUID ruuid, Transaction transaction) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to insert transaction for " + ruuid.toString());
    }
  }
  ...
  public void deleteAllEntries(UUID ruuid) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to delete all entries for " + ruuid.toString());
    }
  }
}
