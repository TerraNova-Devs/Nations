...
public class NationsDAO {
  ...
  public static void createNation(Nation nation) {
    String sqlInsertNation =
        "INSERT INTO nations_table (NUUID, name, banner_base64) VALUES (?, ?, ?)"
            + " ON DUPLICATE KEY UPDATE name = VALUES(name), banner_base64 = VALUES(banner_base64)";
    String sqlInsertSettlement =
        "INSERT INTO settlement_nation_relations (`SUUID`, `NUUID`, `rank`) VALUES (?, ?, ?)"
            + " ON DUPLICATE KEY UPDATE `rank` = VALUES(`rank`)";
    String sqlInsertLeader =
        "INSERT INTO nation_ranks (`PUUID`, `SUUID`, `NUUID`, `rank`) VALUES (?, ?, ?, ?) "
            + "ON DUPLICATE KEY UPDATE `rank` = VALUES(`rank`)";
  ...
  public static void saveNation(Nation nation) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
      ...
      nation
          ...
          .forEach(
              (playerId, rank) ->
                  RegionManager.retrievePlayersSettlement(playerId)
                      .ifPresent(
                          settleId ->
                              savePlayerRankToNation(
                                  rank, playerId, settleId.getId(), nation.getId())));
  ...
  public static void deleteNation(UUID nationId) {
    ...
    try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
        ...
        PreparedStatement psDeleteSettlementRelations =
  ...
  public static void addSettlementToNation(SettlementNationRelation relation) {
    String sql =
        "INSERT INTO settlement_nation_relations (`SUUID`, `NUUID`, `rank`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `rank` = VALUES(`rank`)";
  ...
  private static void saveNationRelations(Nation nation) {
    ...
    String sqlInsert =
        "INSERT INTO nation_relations (`NUUID1`, `NUUID2`, `relation`) VALUES (?, ?, ?)";
  ...
  public static void savePlayerRankToNation(
      NationPlayerRank rank, UUID playerId, UUID settleId, UUID nationId) {
    String sql =
        "INSERT INTO nation_ranks (`PUUID`, `SUUID`, `NUUID`, `rank`) VALUES (?, ?, ?, ?) "
            + "ON DUPLICATE KEY UPDATE `rank` = VALUES(`rank`)";
