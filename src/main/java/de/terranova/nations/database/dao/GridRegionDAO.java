...
public class GridRegionDAO {
  ...
  static {
    queries.put(
        "insert",
        "INSERT INTO `grid_regions` (`RUUID`, `name`, `type`, `location`) VALUES (?, ?, ?, ?);");
    ...
    queries.put(
        "insertParent",
        "INSERT INTO `parent_table` (`RUUID`, `PUUID`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `PUUID` = VALUES(`PUUID`);");
  }
  ...
  public static void insertRegion(GridRegion gridRegion) {
    ...
    try (Connection conn = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to insert grid region: " + gridRegion.getName());
    }
  }
      ...
      while (rs.next()) {
        ...
        if (parentId == null) {
          region = RegionRegistry.createFromArgs(
                  rs.getString("type"),
                  List.of(rs.getString("name"), id, new Vectore2(rs.getString("location")).asString()));
        } else {
          region = RegionRegistry.createFromArgs(
                  rs.getString("type"),
                  List.of(rs.getString("name"), id, new Vectore2(rs.getString("location")).asString(),
  ...
  public static UUID fetchParent(UUID regionId) {
    ...
    try (Connection conn = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("SQL error while fetching parent for region: " + regionId);
      e.printStackTrace();
    }
  ...
  public static void insertParent(UUID regionId, UUID parentId) {
    ...
    try (Connection conn = NationsPlugin.hikari.dataSource.getConnection();
      ...
      NationsPlugin.plugin
          .getLogger()
          .severe("Failed to insert parent for region " + regionId + ": " + e.getMessage());
      e.printStackTrace();
    }
  }
}
