...
public class PropertyAccessDAO {
    ...
    static {
        queries.put("getAll", "SELECT * FROM `access` WHERE RUUID = ? AND (access = 'OWNER' OR access = 'MEMBER');");
    ...
    public static Map<UUID, PropertyAccessLevel> getMembersAccess(UUID ruuid) {
        ...
        Map<UUID, PropertyAccessLevel> access = new HashMap<>();
        try (Connection con = NationsPlugin.hikari.dataSource.getConnection();
             PreparedStatement statement = con.prepareStatement(sql)) {
            statement.setString(1, ruuid.toString());
            ResultSet rs = statement.executeQuery();
            while (rs.next()) {
                access.put(UUID.fromString(rs.getString("PUUID")), PropertyAccessLevel.valueOf(rs.getString("access")));
            }
