...
public class NationCommands extends AbstractCommand {
    ...
    public boolean deleteNation(Player p, String[] args) {
        ...
        if (nation == null) {
            p.sendMessage(Chat.errorFade("Die Nation " + StringUtils.capitalise(nationName) + " existiert nicht."));
            return false;
        }
        ...
        p.sendMessage(Chat.greenFade("Bist du sicher, dass du die Nation " + StringUtils.capitalise(nationName) + " löschen möchtest? Dann gib /nation delete confirm ein."));
        return true;
    }
    ...
    public boolean deleteNationConfirm(Player p, String[] args) {
        ...
        p.sendMessage(Chat.greenFade("Die Nation " + StringUtils.capitalise(nation.getName()) + " wurde erfolgreich gelöscht."));
        return true;
    }
    ...
    public boolean inviteSettlement(Player p, String[] args) {
        ...
        if (settleOpt.isEmpty()) {
            p.sendMessage(Chat.errorFade("Die Stadt " + StringUtils.capitalise(settleName) + " existiert nicht."));
            return false;
        }
        ...
        if (nation.getPlayerRank(p.getUniqueId()).getWeight() < NationPlayerRank.VICE_LEADER.getWeight()) {
            p.sendMessage(Chat.errorFade("Du musst mindestens Vize-Anführer sein um eine Stadt in die Nation einzuladen."));
            return false;
        }

        if (nation.hasSettlement(settle.getId())) {
            p.sendMessage(Chat.errorFade("Die Stadt " + StringUtils.capitalise(settleName) + " gehört bereits zu deiner Nation."));
            return false;
        }

        boolean isSettlementInNation = nationManager.isSettlementInNation(settle.getId());
        if (isSettlementInNation) {
            p.sendMessage(Chat.errorFade("Die Stadt " + StringUtils.capitalise(settleName) + " gehört bereits zu einer Nation."));
            return false;
        }

        if (pendingInvites.containsKey(nation.getId())) {
            p.sendMessage(Chat.errorFade("Die Stadt " + StringUtils.capitalise(settleName) + " hat bereits eine Einladung."));
            return false;
        }
    ...
    public boolean acceptInvite(Player p, String[] args) {
        ...
        if (!pendingInvites.containsKey(nation.getId())) {
            p.sendMessage(Chat.errorFade("Du hast keine Einladung von der Nation " + StringUtils.capitalise(nationName) + "."));
            return false;
        }

        Optional<SettleRegion> settleOpt = RegionManager.retrieveRegion("settle", pendingInvites.get(nation.getId()));
        ...
        nation.broadcast("Die Stadt " + StringUtils.capitalise(settle.getName()) + " ist der Nation " + StringUtils.capitalise(nation.getName()) + " beigetreten.");
        return true;
    }
    ...
    public boolean kickSettlement(Player p, String[] args) {
        ...
        if (nation.getPlayerRank(p.getUniqueId()).getWeight() < NationPlayerRank.VICE_LEADER.getWeight()) {
            p.sendMessage(Chat.errorFade("Du musst mindestens Vize-Anführer sein um eine Stadt aus der Nation zu entfernen."));
            return false;
        }

        if (!nation.hasSettlement(settle.getId())) {
            p.sendMessage(Chat.errorFade("Die Stadt " + StringUtils.capitalise(settleName) + " gehört nicht zu deiner Nation."));
            return false;
        }
