...
public class AccessCommands {
  ...
  public boolean invitePlayer(Player p, String[] args) {
    ...
    if (access.getAccess().getAccessLevel(target.getUniqueId()) != null) {
      p.sendMessage(
          Chat.errorFade(
              String.format(
                  "Der Spieler %s ist bereits Mitglied deiner Stadt.", target.getName())));
      return false;
    }
    if (invites.containsKey(target.getUniqueId())) {
      if (invites.get(target.getUniqueId()).equals(cache.getRegion().getId())) {
        p.sendMessage(
            Chat.errorFade(
                String.format("Der Spieler %s ist bereits eingeladen.", target.getName())));
        return false;
      }
    }
  ...
  public boolean acceptPlayer(Player p, String[] args) {
    ...
    p.sendMessage(
        Chat.greenFade(
            "Du bist erfolgreich der Stadt " + cache.getRegion().getName() + " beigetreten."));
  ...
  public boolean removePlayer(Player p, String[] args) {
    ...
    if (access.getAccess().getAccessLevel(target.getUniqueId()) == null) {
      p.sendMessage(
          Chat.errorFade(
              String.format("Der Spieler %s ist kein Mitglied deiner Stadt.", target.getName())));
      return false;
    }

    if (access.getAccess().getAccessLevel(target.getUniqueId()).getWeight()
        >= access.getAccess().getAccessLevel(p.getUniqueId()).getWeight()) {
      p.sendMessage(
          Chat.errorFade("Du kannst keinen Spieler entfernen der höher gleich du im Rang ist."));
      return false;
    }
  ...
  public boolean leave(Player p, String[] args) {
    ...
    if (Access.hasAccess(cache.getAccess(), AccessLevel.MAJOR)
        || Access.hasAccess(cache.getAccess(), AccessLevel.ADMIN)) {
    ...
    access
        .getAccess()
        .broadcast(
            p.getName() + " hat die Stadt " + cache.getRegion().getName() + " verlassen.",
            AccessLevel.CITIZEN);
    TerraSelectCache.renewSelect(p);
    return true;
  }
  ...
  public boolean rankPlayer(Player p, String[] args) {
    ...
    if (!Access.hasAccess(access.getAccess().getAccessLevel(p.getUniqueId()), AccessLevel.VICE)) {
      p.sendMessage(
          Chat.errorFade("Du musst mindestens Vize sein um access level ändern zu können."));
      return false;
    }
    ...
    p.sendMessage(
        Chat.greenFade(
            String.format(
                "Du hast %s erfolgreich auf den Rang %s gestuft.",
                target.getName(), newAccess.name())));
    TerraSelectCache.renewSelect(target);
    return true;
  }
