...
/**
 * A PlayerAwarePlaceholder that caches the results of a Function<UUID, List<String>> for a
 * specified duration (in milliseconds). Each player (UUID) maintains its own cache entry.
 */
public class CachedUUIDPlaceholder implements PlayerAwarePlaceholder {
  ...
  /** Per-player cache of computed suggestions. Key = UUID, Value = last computed suggestions */
   ...
   * @param cacheDurationMillis How long (in milliseconds) each UUID’s cached suggestions remain
   *     valid.
   */
