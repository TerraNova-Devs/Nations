...
/**
 * A supplier implementation that caches the result from a delegate supplier for a specified
 * duration. This class can be used to prevent frequent expensive computations or I/O operations by
 * caching the result and reusing it for a defined time period.
 *
 * <p><b>Usage Example:</b>
 *
 * <pre>{@code
...
public class CachedSupplier<T> implements Supplier<T> {
  ...
  /**
   * Constructs a new CachedSupplier that caches values from the given delegate supplier for the
   * specified cache duration.
   *
   * @param delegate The underlying supplier that provides the fresh data.
   * @param cacheDurationMillis The duration (in milliseconds) that the cache is valid before
   *     needing a refresh.
   */
  ...
  /**
   * Returns the cached value if it is still valid; otherwise, obtains a fresh value from the
   * delegate supplier, updates the cache, and returns the new value.
   *
