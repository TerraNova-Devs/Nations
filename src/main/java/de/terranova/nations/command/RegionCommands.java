...
public class RegionCommands {
  ...
  public static boolean createRegion(Player p, String[] args) {
    ...
    String name =
        MiniMessage.miniMessage()
            .stripTags(String.join("_", Arrays.copyOfRange(args, 3, args.length)));
      ...
      p.sendMessage(
          Chat.errorFade(
              String.format(
                  ...
                  RegionRegistry.factories.keySet())));
      return false;
    }
  ...
  public static boolean removeRegion(Player p, String[] args) {
    ...
    if (!Access.hasAccess(playerAccess, AccessLevel.MAJOR)) {
      p.sendMessage(
          Chat.errorFade("You do not have the required access level to remove this settlement."));
      return false;
    }
    region.remove();
  ...
  public static boolean claimRegion(Player p, String[] args) {
    ...
    if (!(cache.getRegion() instanceof GridRegion region)) {
      p.sendMessage(
          Chat.errorFade("Du kannst die ausgewählte region nicht durch claimen erweitern"));
      return false;
    }
