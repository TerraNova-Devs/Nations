...
/**
 * Verwaltet alle Profession-bezogenen Infos einer Stadt. Lädt Status, Objectives, Buildings etc.
 * und wendet Logik an.
 */
public class ProfessionProgressManager {
  ...
  public static ProfessionProgressManager loadForSettlement(UUID settlementId) {
    ...
    Map<String, ProfessionStatus> map =
        SettlementProfessionRelationDAO.getAllStatuses(settlementId.toString());
    ...
    mgr.activeProfessionId =
        SettlementProfessionRelationDAO.getActiveProfessionID(settlementId.toString());
  ...
  /**
   * Gibt den "echten" Status einer Profession zurück, ggf. dynamisch berechnet (LOCKED ->
   * AVAILABLE), etc.
   */
  public ProfessionStatus getProfessionStatus(String professionId) {
    ...
    if (p.getLevel() > 1) {
      ...
      if (prev != null) {
        ProfessionStatus prevStat =
            professionStatuses.getOrDefault(prev.professionId, ProfessionStatus.LOCKED);
  ...
  public boolean completeProfession(String professionId) {
    ...
    if (SettlementProfessionRelationDAO.getStatus(settlementId.toString(), professionId)
        .equals(ProfessionStatus.COMPLETED)) {
  ...
  /**
   * Wenn wir eine Profession aktivieren, wollen wir evtl. alle anderen, die "ACTIVE" sind, auf
   * "PAUSED" setzen.
   */
